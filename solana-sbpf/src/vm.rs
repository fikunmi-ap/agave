...
use crate::{
    ...
    memory_region::{MemoryMapping, MemoryState},
...
pub const ADDRESS_MAPS: usize = 5;
...
pub enum RuntimeEnvironmentSlot {
    ...
    /// [EbpfVm::registers]
    Registers = 7,
    /// [EbpfVm::AddressMapping]
    AddressMapping = 19,
    /// [EbpfVm::program_result]
    ProgramResult = 19 + ADDRESS_MAPS as isize * 2,
    /// [EbpfVm::memory_mapping]
    MemoryMapping = 27 + ADDRESS_MAPS as isize * 2,
}
...
pub struct EbpfVm<'a, C: ContextObject> {
    ...
    /// AddressMapping for 4 regions.
    /// This is not a struct to simplify the generate code.
    address_len_ro: [u32; ADDRESS_MAPS],
    address_len_rw: [u32; ADDRESS_MAPS],
    address_offset: [u64; ADDRESS_MAPS],
...
impl<'a, C: ContextObject> EbpfVm<'a, C> {
    ...
    pub fn new(
        ...
        // fill the address mapping
        let mut address_len_ro = [0; ADDRESS_MAPS];
        let mut address_len_rw = [0; ADDRESS_MAPS];
        let mut address_offset = [0; ADDRESS_MAPS];
        for m in memory_mapping.get_regions() {
            let slot = (m.vm_addr >> 32) as usize;
            // Combine multiple adjacent slots used for the input accounts.
	    // This assumes the slots are copied into the same buffer and are not directly mapped. 
            address_len_ro[slot] = core::cmp::max(address_len_ro[slot], m.vm_addr_end as u32);
            if m.state == MemoryState::Writable.into() {
                address_len_rw[slot] = address_len_ro[slot];
            }
            if m.vm_addr & 0xffff_ffff == 0 {
                address_offset[slot] =
                    m.host_addr.clone().into_inner() - (((m.vm_addr >> 32) as u64) << 32);
            }
        }
