...
use crate::{
    ...
    vm::{
        get_runtime_environment_key, Config, ContextObject, EbpfVm, RuntimeEnvironmentSlot,
        ADDRESS_MAPS,
    },
    x86::{
...
const ANCHOR_INVALID_MEMORY: usize = 21;
const ANCHOR_COUNT: usize = 22; // Update me when adding or removing anchors
...
/// RBP: FramePointer offset
const REGISTER_FRAME_PTR: X86Register = CALLEE_SAVED_REGISTERS[0];
...
impl<'a, C: ContextObject> JitCompiler<'a, C> {
    ...
    /// Translate the address from the given registers. This returns the offset register.
	fn emit_address_translation(&mut self, addr: X86Register, load: bool, shortcut: bool) -> X86Register {
	    // Can short the translation as the stack access can be statically proven.
	    if shortcut && addr == REGISTER_MAP[FRAME_PTR_REG] {
		return  REGISTER_FRAME_PTR;
	    }
	// flip the upper and lower bits: scratch = addr >> 32
        self.emit_ins(X86Instruction::mov(OperandSize::S64, addr, REGISTER_SCRATCH));
	self.emit_ins(X86Instruction::alu_immediate(OperandSize::S64, 0xc1, 5, REGISTER_SCRATCH, 32, None));
	// compare against the number of address maps to avoid overflows
        self.emit_ins(X86Instruction::cmp_immediate(OperandSize::S32, REGISTER_SCRATCH, ADDRESS_MAPS as i64, None));
	self.emit_ins(X86Instruction::conditional_jump_immediate(0x83, self.relative_to_anchor(ANCHOR_INVALID_MEMORY, 6)));


	let offset = self.slot_in_vm(RuntimeEnvironmentSlot::AddressMapping);

	// length check
	let indirect = X86IndirectAccess::OffsetIndexShift(offset + if load {
	    0
	} else {
	    4 * ADDRESS_MAPS as i32
	}, REGISTER_SCRATCH, 2);
	self.emit_ins(X86Instruction::cmp(OperandSize::S32, addr, REGISTER_PTR_TO_VM, Some(indirect)));
	self.emit_ins(X86Instruction::conditional_jump_immediate(0x82, self.relative_to_anchor(ANCHOR_INVALID_MEMORY, 6)));

	// get the offset
	let indirect = X86IndirectAccess::OffsetIndexShift(offset + 8 * ADDRESS_MAPS as i32, REGISTER_SCRATCH, 3);
        self.emit_ins(X86Instruction::alu(OperandSize::S64, 0x8b, REGISTER_SCRATCH, REGISTER_PTR_TO_VM, Some(indirect)));
	REGISTER_SCRATCH
    }

    fn emit_load<const LEN: usize>(&mut self, dst: X86Register, addr: X86Register, ofs: i16) {
	let offset = self.emit_address_translation(addr, true, true);
	let indirect = X86IndirectAccess::OffsetIndexShift(ofs.into(), offset, 0);
	match LEN {
            1 => self.emit_ins(X86Instruction::load(OperandSize::S8,  addr, dst, indirect)),
            2 => self.emit_ins(X86Instruction::load(OperandSize::S16, addr, dst, indirect)),
            4 => self.emit_ins(X86Instruction::load(OperandSize::S32, addr, dst, indirect)),
            8 => self.emit_ins(X86Instruction::load(OperandSize::S64, addr, dst, indirect)),
	    _ => unreachable!(),
	}
    }
	
    fn emit_store_imm<const LEN: usize>(&mut self, addr: X86Register, ofs: i16, value: i64) {
	let offset = self.emit_address_translation(addr, false, true);
	let indirect = X86IndirectAccess::OffsetIndexShift(ofs.into(), offset, 0);
	match LEN {
            1 => self.emit_ins(X86Instruction::store_immediate(OperandSize::S8, addr, indirect, value)),
            2 => self.emit_ins(X86Instruction::store_immediate(OperandSize::S16, addr, indirect, value)),
            4 => self.emit_ins(X86Instruction::store_immediate(OperandSize::S32, addr, indirect, value)),
            8 => self.emit_ins(X86Instruction::store_immediate(OperandSize::S64, addr, indirect, value)),
            _ => unreachable!(),
	}
    }
    fn emit_store<const LEN: usize>(&mut self, addr: X86Register, ofs: i16, src : X86Register) {
	let offset = self.emit_address_translation(addr, false, true);
	let indirect = X86IndirectAccess::OffsetIndexShift(ofs.into(), offset, 0);
        match LEN {
            1 => self.emit_ins(X86Instruction::store(OperandSize::S8, src, addr, indirect)),
            2 => self.emit_ins(X86Instruction::store(OperandSize::S16, src, addr, indirect)),
            4 => self.emit_ins(X86Instruction::store(OperandSize::S32, src, addr, indirect)),
            8 => self.emit_ins(X86Instruction::store(OperandSize::S64, src, addr, indirect)),
            _ => unreachable!(),
	}
    }
	...
	// Address translation for load/store failed
        self.set_anchor(ANCHOR_INVALID_MEMORY);
        self.emit_set_exception_kind(EbpfError::InvalidVirtualAddress(0));
        self.emit_ins(X86Instruction::jump_immediate(self.relative_to_anchor(ANCHOR_THROW_EXCEPTION, 5)));
	...
	// Translate the frame-ptr register
	let reg = self.emit_address_translation(REGISTER_MAP[FRAME_PTR_REG], false, false);
	self.emit_ins(X86Instruction::mov(OperandSize::S64, reg, REGISTER_FRAME_PTR));
